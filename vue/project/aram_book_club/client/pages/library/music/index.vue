<template>
  ...
  >
    ...
    <!-- option -->
    <div class="option">
      <select ref="selCategory" v-model="selCateValue" @change="selCategory">
        <option v-for="(c, index) in category" :key="index" :value="c.prodId">{{
          c.prodName
        }}</option>
      </select>
      <div class="search">
      ...
      <tbody v-if="musicList.length <= 0">
        <tr>
          <td colspan="5" class="no-list">
            <p>검색결과가 존재하지 않습니다.</p>
            <button class="btn-return" @click="returnList">음원 보기</button>
          </td>
        </tr>
      </tbody>
      <tbody v-else>
...
export default {
  ...
  updated() {
    if (this.selCateValue === '' && this.$refs.selCategory.options[0]) {
      this.$refs.selCategory.options[0].selected = 'selected'
    }
  },
  watch: {
    viewMode(n, p) {
  ...
  methods: {
    ...
    async getCategory() {
      try {
        const { data } = await this.$axios.get('/library/sounds/products')
    ...
    async selCategory() {
      let prodId
      if (this.selCateValue === '') {
        prodId = this.category[0].prodId
      } else {
        prodId = this.selCateValue
      }
      ...
      try {
        const { data } = await this.$axios.get(
          `/library/sounds/products/${prodId}`
        )
    ...
    async searchList() {
      ...
      try {
        const { data } = await this.$axios.get(
          `/library/sounds/products/${this.$refs.selCategory.value}`,
          {
            params: {
