<template>
  ...
  >
    <div class="prod-info">
      ...
      <div class="info">
        ...
        <ul>
          ...
          <li>
            <span class="tit-info">교육 영역</span>
            <p class="txt-info">{{ prodInfo.eduAraNm }}</p>
          </li>
          <li v-if="prodInfo.existRelatedProd === 'Y'">
            <span class="tit-info">연계 전집</span>
            <p class="txt-info">{{ prodInfo.relatedProdNm }}</p>
          </li>
          <!-- <li v-if="prodInfo.dgtlProdPriceList">
            <span class="tit-info">상품 가격</span>
            <p class="txt-info">
              {{
                prodInfo.dgtlProdPriceList[0].price
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ',')
              }}원/{{ prodInfo.dgtlProdPriceList[0].period }}
            </p>
          </li> -->
        </ul>
      </div>
    ...
    <div v-else class="btn-wrap">
      ...
      <button
        v-if="prodInfo.usedTrlvrsn === 'N' && prodInfo.payYn === 'N'"
        @click="openLinkMap('/babyall', prodInfo)"
        class="btn-exp"
      >
        체험판 시작하기
      </button>
      <button
        v-if="
          prodInfo.usedTrlvrsn === 'Y' &&
            prodInfo.remainTrlvrsnDays >= 1 &&
            prodInfo.payYn === 'N'
        "
        @click="openLink('/babyall', prodInfo)"
        class="btn-exp"
      >
        체험하기 ({{ prodInfo.remainTrlvrsnDays }}일 남음)
      </button>
      <button
        v-if="
          prodInfo.usedTrlvrsn === 'Y' &&
      ...
      <p v-if="prodInfo.payYn === 'N'" class="txt-day">
        - 체험판은 사용 시작일로부터 14일간 이용 가능합니다.
      </p>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getProdDetail() {
      try {
        const { data } = await this.$axios.get(
          `/prod/digital/${this.$route.params.id}`,
          {
            params: {
    ...
    reviewOpen(index) {
      this.review[index].open = !this.review[index].open
      this.$forceUpdate()
    },
    // 체험판 종료
    testEnd() {
