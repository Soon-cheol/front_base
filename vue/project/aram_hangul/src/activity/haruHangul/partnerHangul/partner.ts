...
const api = `${config.restAPI}viewer/partner/sounds/`
let gameData = null

const correctAry: Array<SideExam> = []
const wrongAry: Array<SideExam> = []

const quizAry: Array<Phoneme> = []
...
export class BgCharacter extends PIXI.Container {
  ...
  destroy() {
    this.move = () => null
    this.mCharacterMotion.kill()
    this.mCharacterMotion = null
    this.mCloudMotion.kill()
...
export class Phoneme extends PIXI.Container {
  ...
  focus() {
    ...
    // 초성//중성//종성
    const sndType = getSyllableType(this.mData.phoneme)
    ...
    this.on('pointertap', () => {
      ...
      console.log(this.mData.phoneme)
      console.log(`${sndType}${num}.mp3`)
      sound.play()
  ...
  complete() {
    // const phonemeType = getSyllableType(this.mData.phoneme)
    // const num = syllableToNum(this.mData.phoneme)

    // console.error(`${phonemeType}${num}.mp3`)
    ...
    // gsap.to(this.mDeep, { alpha: 1, duration: 0.5 })
    // gsap.to(this.mDefault, { alpha: 1, duration: 0.5 })
  }
  ...
  createText() {
    ...
    // 초성 중성 종성
    const phonemeType = getSyllableType(p)
    // ex=> 아=> type1 , 오=>type2
    const type = `t${t}_`
    // 음소에 매치된 숫자 가져오기
  ...
  startGame(): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      for (let i = 0; i < this.mText.length; i++) {
        ...
        console.warn(`text => ${this.mText[i]}`)
...
export class SideExam extends PIXI.Sprite {
  ...
  private mText: PIXI.Sprite
  constructor(type: number, index: number, phoneme: string) {
    super()
    ...
    this.mBg = new PIXI.Sprite(
      ResourceManager.Handle.getCommonResource(
        `side_type${type}_pink${index + 1}.png`
      ).texture
    )
    this.mBg.anchor.set(0.5)

    const imgType = getSyllableType(phoneme)
    const num = syllableToNum(phoneme)
    this.mText = new PIXI.Sprite(
      ResourceManager.Handle.getCommonResource(
        `${imgType}t${type}_${num}.png`
      ).texture
    )
...
export class SideBar extends PIXI.Sprite {
  ...
  constructor() {
    ...
    for (let i = 0; i < gameData['correct'].length; i++) {
      ...
      for (let j = 0; j < ary.length; j++) {
        console.log(type, j, ary[j])
        correctAry.push(new SideExam(type, j, ary[j]))
        wrongAry.push(new SideExam(type, j, gameData.wrong[index]))
        index += 1
      }
    }
  ...
  async startEvent() {
    this.mCorrect.onClick = async () => {
      ...
      ;(this.parent as GameScene).gameStep += 1
      this.mStep += 1
...
export class GameScene extends PIXI.Container {
  ...
  async createQuiz() {
    ...
    this.mSideBar.x = config.w
    gsap.to(this.mSideBar, { x: config.w - this.mSideBar.width, duration: 0.5 })
...
export class Partner extends SceneBase {
  ...
  wrong() {
    this.mWrongCount += 1
    console.log(
      `%c 총합(듣,읽,쓰)오답갯수=>[${this.mWrongCount}]`,
  ...
  getData(): Promise<void> {
    return new Promise<void>((resolve) => {
      const requestUrl = `${config.restAPI}data/partner_data.json`
